...
def get_help_buttons():
    ...
    Markup.append(
        [Button.inline("- إغـلاق -", data="close")],
    )
...
async def help_cmd(event: NewMessage.Event):
    ...
    if not LIST.get(module):
        return await event.eor(f"**⎆ القائمة {module} غير موجودة أصـلًا** .")
...
async def help_func(moh):
    ...
    if key == "vcbot" and not plugs:
        return await moh.answer("**◙ يجب عليك إضافة متغير السمـاع الصوتي**", alert=True)
    if "|" in count:
        _, count = count.split("|")
...
async def on_plug_in_callback_query_handler(event):
    await event.edit(
        "**⎆ تم إغلاق القائمة الرئيسية بنجاح**",
...
async def _(e):
    if not InlinePlugin:
        return await e.answer("**⎆ لا يتوفر أوامر أنلاين حاليًّــا**", alert=True)
    _InButtons = [
