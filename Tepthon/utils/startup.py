...
async def tag_chat():
    ...
    if not tag_chat:
        ...
        try:
            group_id = await jmubot(
                CreateChannelRequest(
                    title="مجموعة التخزين - تيبثون",
                    about="مجموعة تخزين الرسائل الخاصة بك\n\n انضم لقناة السورس @Tepthon",
                    megagroup=True,
                ),
            )
        except ChannelsTooMuchError as er:
